CREATE TABLE Movie(
	...
	PRIMARY KEY(id) ENGINE=INNODE);--Movie ID must be unique

CREATE TABLE Actor(
	id int COMMENT 'Actor ID', 
	...
	CHECK (dob<dod) ENGINE=INNODE);--Date of death cannot be earlier than date of birth
...
CREATE TABLE MovieGenre(
	...
	FOREIGN KEY (mid) references Movie(id) ENGINE=INNODE);--mid must match a id in Movie table

CREATE TABLE MovieDirector(
	mid int COMMENT 'Movie ID', 
	did int COMMENT 'Director ID',
	FOREIGN KEY (mid) references Movie(id), --mid must match a id in Movie table
	FOREIGN KEY (did) references Director(id) ENGINE=INNODE);--did must match a id in Director table
...
CREATE TABLE MovieActor(
	...
	FOREIGN KEY (mid) references Movie(id), --mid must match a id in Movie table
	FOREIGN KEY (aid) references Actor(id) ENGINE=INNODE); --aid must match a id in Actor table
...
CREATE TABLE MaxPersonID(id int COMMENT 'Max ID assigned to all persons' ENGINE=INNODE);

CREATE TABLE MaxMovieID(id int COMMENT 'Max ID assigned to all movies' ENGINE=INNODE);
